...
for (var i = 0; i < obj_DC_player.total_perks; i++) {
    var _options = [];
    
    // Build list of valid perks
    for (var _i = 0; _i < PERKS.COUNT; _i++) {
        if (obj_DC_player.perks[_i] < obj_DC_player.limit[_i]) {
    ...
    instance_create_depth(obj_DC_player.x,obj_DC_player.y, 1,obj_DC_perk,{upgrade_num: i, perk: _options[0] }  );
}
