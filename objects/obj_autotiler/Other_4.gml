...
with(obj_wall_basic){
    ...
    for (i = 1; i < tiles_w - 1; i++) {
		if(!instance_position(bbox_left + i * tile_sz,bbox_top-1,obj_wall_basic)){
    ...
    // Bottom middle
    for (i = 1; i < tiles_w - 1; i++) {
		if(!instance_position(bbox_left + i * tile_sz, bbox_bottom+1,obj_wall_basic)){
