if (cutscene_active) {
    if (!waiting) {
        ...
        switch (action.type) {
			...
			// Move Tet (Kinda unused, fix later)
			case ACTION.MOVE_TET:
				tet_goto(action.target_x,action.target_y,action.spd);
				if(abs(obj_tet.x - action.target_x) < 10 && abs(obj_tet.y - action.target_y) < 10){
					current_action += 1;	
				}
			break;
			
			// Tet speaking
			case ACTION.TET_SPEAK:
			var newTxt = [];
			...
			//Set the state of Tet
			case ACTION.SET_TET:
					obj_tet.state = action.state;
					obj_tet.eyelid_no = action.eyelids;
