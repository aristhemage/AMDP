...
if (jump_buffer_timer > 0 && (on_ground || coyote_timer > 0)) {
	...
	x_scale_vis = 0.75;
	squish = true;
	audio_play_sound(choose(snd_jump1,snd_jump2,snd_jump3,snd_jump4),100,false,obj_settings_master.sfx_volume*5);	
	repeat(4){
...
if(key_jetpack && jetpack_fuel > 0){
	...
	if(!audio_is_playing(snd_jetpack))
		audio_play_sound(snd_jetpack,1,true,obj_settings_master.sfx_volume*5);
...
if keyboard_check_released(ord("I")){debug_invince = !debug_invince}
